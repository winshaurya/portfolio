...
const SelectedWorks = () => {
  const projects = [
    { slug: "leadgs", title: "LeadGS Leaderboard", desc: "College Competitive Programming Leaderboard managing 100+ active users with weighted scoring algorithms.", live: "https://lb.sgsits.ac.in", github: "https://github.com/winshaurya/LeadGS", image: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=1600&auto=format&fit=crop", gradient: "from-violet-500 via-fuchsia-400/60 via-indigo-500/60" },
    { slug: "bitchess", title: "bit.Chess Platform", desc: "Real-Time Multiplayer Chess Platform with spectator support via WebSockets and Express.js.", live: "https://bit-chess.onrender.com/", github: "https://github.com/winshaurya/bit.chess", image: "https://images.unsplash.com/photo-1529699211952-734e80c4d42b?q=80&w=1600&auto=format&fit=crop", gradient: "from-sky-500 via-blue-400/60" },
    { slug: "movierec", title: "Movie Recommender", desc: "Recommendation system over TMDB Movies dataset, achieving 97% accuracy via Streamlit.", live: "https://moviereccomendationsystem-9669.streamlit.app/", github: "https://github.com/winshaurya/MovieReccomendationSystem", image: "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?q=80&w=1600&auto=format&fit=crop", gradient: "from-emerald-500 via-emerald-300/60 via-teal-500/60" },
...
const Explorations = () => {
    ...
    fetch('https://api.github.com/users/winshaurya/repos?sort=updated&per_page=100')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          const withHomepage = data.filter(r => !r.fork && r.homepage && r.homepage.startsWith('http'));
          // Randomize array
          const shuffled = withHomepage.sort(() => 0.5 - Math.random());
          // Take top 6 for the layout
          setLiveProjects(shuffled.slice(0, 6));
        }
      })
  ...
  const defaultImages = [
    { id: 1, title: "Celestial Planets", img: "https://images.unsplash.com/photo-1614730321146-b6fa6a46bcb4?q=80&w=600&auto=format&fit=crop", url: "#" },
    { id: 2, title: "ASCII Art Study", img: "https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=600&auto=format&fit=crop", url: "#" },
    { id: 3, title: "Atmospheric Smoke", img: "https://images.unsplash.com/photo-1580130080644-8d96a6c4c04a?q=80&w=600&auto=format&fit=crop", url: "#" },
    { id: 4, title: "Abstract Cylinder", img: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=600&auto=format&fit=crop", url: "#" },
    { id: 5, title: "Organic Waves", img: "https://images.unsplash.com/photo-1550684376-efcbd6e3f031?q=80&w=600&auto=format&fit=crop", url: "#" },
    { id: 6, title: "Geometric Cubes", img: "https://images.unsplash.com/photo-1500462918059-b1a0cb512f1d?q=80&w=600&auto=format&fit=crop", url: "#" },
  ];

  // Map fetched repos to the display format, fallback to defaultImages while loading
  const displayItems = liveProjects.length > 0 ? liveProjects.map(repo => ({
    id: repo.id,
    title: repo.name,
...
const ContributionGraph = () => {
  ...
  useEffect(() => {
    // Generate mock data mimicking GitHub API
    const weeks = [];
    let total = 0;
    const now = new Date();
    ...
    for (let i = 52; i >= 0; i--) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        if (r > 0.85) count = Math.floor(Math.random() * 6) + 3;
        if (r > 0.95) count = Math.floor(Math.random() * 10) + 6;
  ...
  return (
    <section className="mt-12 opacity-80 hover:opacity-100 transition-opacity duration-500 relative max-w-4xl mx-auto w-full overflow-x-auto pb-4 custom-scrollbar">
      {tooltip && (
        <div 
          className="fixed z-50 px-3 py-2 text-xs font-medium text-white bg-surface border border-stroke rounded-md shadow-xl pointer-events-none whitespace-nowrap"
          style={{ left: tooltip.x, top: tooltip.y - 40, transform: "translateX(-50%)" }}
        >
          <span className="text-muted mr-2">{tooltip.date}</span>
          <span className="text-text-primary">{tooltip.count} contributions</span>
        </div>
      )}
              ...
              {week.contributionDays.map((day: any, j: number) => (
                <div
                  ...
                  onMouseEnter={(e) => {
                    ...
                    setTooltip({ date: day.date, count: day.contributionCount, x: rect.left + rect.width / 2, y: rect.top });
                  }}
